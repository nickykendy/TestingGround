...
void AFirstPersonCharacter::BeginPlay()
{
	...
	if (World != nullptr && Mesh1P != nullptr && GunClass != nullptr) {
		const FVector SpawnLocation = Mesh1P->GetSocketLocation(FName("GripPoint"));
		const FRotator SpawnRotation = Mesh1P->GetSocketRotation(FName("GripPoint"));
		Gun = World->SpawnActor<AGun>(
			GunClass,
			SpawnLocation,
			SpawnRotation
		);
